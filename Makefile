...
VERSION ?= 0.1.0
IMAGE_TAG_BASE ?= ghcr.io/fabiendupont/nvidia-bare-metal-manager-operator
...
# OLM bundle channel configuration
...
KIND_CLUSTER ?= carbide-operator-test-e2e
...
STUB_IMG ?= localhost/carbide-stub:e2e
...
docker-buildx: ## Build and push docker image for the manager for cross-platform support
	...
	- $(CONTAINER_TOOL) buildx create --name carbide-operator-builder
	$(CONTAINER_TOOL) buildx use carbide-operator-builder
	- $(CONTAINER_TOOL) buildx build --push --platform=$(PLATFORMS) --tag ${IMG} -f Dockerfile.cross .
	- $(CONTAINER_TOOL) buildx rm carbide-operator-builder
	rm Dockerfile.cross
...
catalog-validate: opm ## Validate the FBC catalog.
